...
The current parent Run remains Status=preparing with `Output Prepared=true`, `Finished At=""`, and `Cache Reconciled=false`. Require exact booleans and enforce `Integration Due must equal Integration Performed`; `force-world-memory` requires true. The exact Report/notification matrix is: performed integration has a nonempty canonical Integration Key, one six-hour Report, no hourly Report, and `Notification Plan=six-hour`; non-integration material change has an empty Integration Key, one hourly Report, no six-hour Report, and `Notification Plan=hourly-briefing`; non-integration nonmaterial has an empty Integration Key, no Report, and `Notification Plan=silent`. The direct Report validator enforces the same per-type parent projection: six-hour requires parent Due/Performed true and six-hour notification; hourly requires parent Due/Performed false, Material Change true, hourly notification, and empty parent/Report Integration Keys.
...
Record `Notification Plan` as `silent|hourly-briefing|six-hour|error`; never record an unobservable `sent` claim. Scheduled nonmaterial/nondue runs are silent. A successful due/forced six-hour Report is visible. A manual Run returns its direct result. Surface source failures and data gaps in visible output without inventing evidence.
