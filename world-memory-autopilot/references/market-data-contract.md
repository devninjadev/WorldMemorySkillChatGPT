...
The collector starts the FRED batch, the HYG/LQD history provider, and all three Binance tickers in one parallel bounded pass. The timeout is the per-member network ceiling and therefore the pass ceiling apart from parsing and a first-use dependency bootstrap. A lagged but valid FRED observation remains usable with its actual date and a lag warning. For net liquidity, use only `netLiquidity.status:"ok"`; when a required component fails, preserve every other successful FRED member and identify only the missing component. Never replace a collector failure with an unattempted or model-invented value.
...
Retrieve daily `Close` history for `HYG` and `LQD` over three months with no automatic adjustment, matching FinanceAgentGUI's `yfinance.download(..., period="3mo", interval="1d", auto_adjust=False)` contract. On first use, check `yfinance`, `pandas`, and `numpy`; if missing, install the pinned skill requirements into the isolated `.world-memory-runtime/deps` directory with the active interpreter, retry once without the pip cache, prepend that exact directory to `sys.path`, invalidate import caches, and re-import all required modules. Never mutate the system interpreter. A dependency-install failure is a source-path failure, not a fabricated observation.

Request HYG and LQD concurrently through `Ticker.history(..., raise_errors=True)` so yfinance does not reduce a provider exception to an empty frame plus a log line. A `YFRateLimitError` is a provider rate limit: do not hammer Yahoo's `query1` and `query2` hosts as immediate retries. When dependency bootstrap itself is unavailable, the plan's exact public `creditRatio.historyUrls` remain the approved no-package fallback and each timestamp maps to `chart.result[0].indicators.quote[0].close`.

Persist the last complete six-session-or-longer paired history in `.world-memory-runtime/market-cache.json`. A cache saved within six hours is a `fresh-hit` and suppresses another hourly Yahoo request. After a live failure, a cache whose latest common observation is no more than seven calendar days old may be used as `stale-fallback`; retain the original observation and fetched times and record the failed live attempt as a degraded data-quality member. An older, future-dated, malformed, partial, or schema-mismatched cache is unusable. A successful live acquisition atomically replaces the cache and is labeled `refreshed`.
