...
Require the `Notion ChatGPT plugin` (the Notion connector for ChatGPT) to be installed, authenticated, and granted access to the target Notion workspace before setup or any operational run. If it is unavailable, stop before accessing or mutating World Memory state and report the missing dependency; do not substitute generic web access.
...
- An active enabled Installation may collect, analyze, write Run/Feed Batch/Report children, and reconcile cache. `Autopilot Enabled=false` blocks Memory revisions and suggestion completion, not collection or reporting.
